...
possible_play(Board,PosList):-invert(Board,[L|_]),empty_pos(L,1,[],PosList).
...
checkWin(NB,_/P/_,_,_/P/'W',(L,C)):-length(NB,X),La is X - L,invert(NB,I),diagonal(I,P,(La,C)).
...
moves(Estado-B-(_),PosList):-possible_play(B,PP),findall(NEstado-NB-(Cord),playAI(B,Estado,NB,NEstado,PP,Cord),PosList).
