...
can(push(S, B, SPos, NewSPos, NewBPos), [at(S, SPos), at(B,NewSPos), clear(NewBPos)]) :-	
	sokoban(S), box(B), adjacent(SPos,NewSPos), newBoxPos(SPos, NewSPos ,NewBPos).

newBoxPos((XS, Y), (XB, Y), (NX, Y)):-NX is XB * 2 - XS.
newBoxPos((X, YS), (X, YB), (X, NY)):-NY is YB * 2 - YS.
...
adjacent(Pos1, Pos2):-n(Pos1, Pos2);n(Pos2,Pos1).         % Cells A and B are adjacent in the grid
